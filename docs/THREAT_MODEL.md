...
This document describes the security boundaries of `nitka` as a local
...
| VPS compromise | SSH hardening, minimal packages, automated updates | `nitka` cannot recover a compromised host |
...
even if `nitka` is no longer maintained.
