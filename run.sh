...
build_if_needed() {
    ...
    current_digest="$(image_label nitka.base-image)"
    current_hash="$(image_label nitka.dockerfile-sha256)"
    current_uid="$(image_label nitka.runtime-uid)"

    if [[ -n "$digest" && "$current_digest" == "$BASE_IMAGE@$digest" && "$current_hash" == "$dockerfile_hash" && "$current_uid" == "$NITKA_UID" ]]; then
        return 0
    fi
    ...
    docker compose -f "$COMPOSE_FILE" build \
        --build-arg "NITKA_BASE_IMAGE=$base_arg" \
        --build-arg "NITKA_DOCKERFILE_SHA256=$dockerfile_hash" \
        nitka
}

build_if_needed
printf '\033[H\033[2J\033[3J'
exec docker compose -f "$COMPOSE_FILE" run --rm --pull never nitka "$@"
