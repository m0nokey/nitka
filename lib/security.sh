...
rotate_ssh_key() {
    ...
    old_pub="$(python3 -c 'import json,sys; print(json.load(sys.stdin)["nodes"][sys.argv[1]]["management"]["authorized_key"], end="")' "$node" <"$before")"
    ...
    python3 -c 'import json,sys; print(json.load(sys.stdin)["nodes"][sys.argv[1]]["management"]["private_key"], end="")' "$node" <"$before" >"$old_key"
    chmod 600 "$old_key"
...
data = {
    "access_xray_state": node["access"]["xray_reality"],
    "access_xray_public_host": node["host"],
    "deploy_user": "deploy",
    "deploy_authorized_key": node["management"]["authorized_key"],
...
PY
    ...
    if ! run_ansible_playbook -i "$inventory" -e "@$extra" -e rotate_remove_old_key=false "$ROOT_DIR/ansible/playbooks/rotate_management_ssh.yml" --private-key "$old_key"; then
        pipeline_abort
        rm -f "$before" "$extra" "$rotated_state"
        rm -rf "$inventory_dir" "$key_dir"
        return 1
    ...
    if ! run_ansible_playbook -i "$inventory" -e "@$extra" -e rotate_remove_old_key=true "$ROOT_DIR/ansible/playbooks/rotate_management_ssh.yml" --private-key "$new_key"; then
